    ...
    post(_this){
        return client.post(this.serviceName,_this.state)
            ...
            .then(data => {                                  
                ...
                if(error.response === undefined)
                {
                    _this.showError("Erro", "Não foi possível salvar o participante.")
                }else{
                    _this.showError("Erro", "Não foi possível salvar o participante. Detalhes: "+error.response.data.message)
                }
            });
    }
}